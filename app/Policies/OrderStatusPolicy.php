...
class OrderStatusPolicy
{
    ...
    {
        return $user->isAdmin() || $orderStatus->customer->hasUser($user->id);
    }
    ...
    public function update(User $user, $data = null)
    {
        ...
        if (isset($data['id']) && $orderStatus = OrderStatus::find($data['id'])) {
            if ($user->hasCustomer($orderStatus->customer_id) == false) {
                return false;
            }
        }
    ...
    public function delete(User $user, $data = null)
    {
        ...
        if (isset($data['id']) && $orderStatus = OrderStatus::find($data['id'])) {
            return $user->hasCustomer($orderStatus->customer_id);
        }
